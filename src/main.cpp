...
int main() {
    ...
    while(1) {
        ...
        getDataINA(&sensor2,&pvVolt,&pvCurr,&pvPower);
        batEnergy=batPower/1000*(float)seconds/3600;
        pvEnergy=pvPower/1000*(float)seconds/3600;
        if(mode==0) menu1_display(true,batVolt/1000,true,true);
        if(mode==1) menu2_display(pvVolt/1000,pvCurr,pvPower/1000,pvEnergy,timerOn1,hour_,min_,sec_);
        if(mode==2) menu3_display(batVolt/1000,batCurr,batPower/1000,batEnergy,timerOn2,hour_,min_,sec_);
        if(batCurr<50) relay.write(1);
        else relay.write(0);
        myled=!myled;
        wait_ms(100);
    }
